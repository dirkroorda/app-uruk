...
def atfFromSign(app, n, flags=False):
  F = app.api.F
  Fs = app.api.Fs
  if F.otype.v(n) != 'sign':
    return '«no sign»'
...
def atfFromQuad(app, n, flags=False, outer=True):
  api = app.api
  E = api.E
  F = api.F
  Fs = api.Fs
  ...
  for child in children:
    ...
    thisResult = (
        app.atfFromQuad(child, flags=flags, outer=False)
        if childType == 'quad' else app.atfFromSign(child, flags=flags)
    )
...
def atfFromOuterQuad(app, n, flags=False):
  api = app.api
  F = api.F
  nodeType = F.otype.v(n)
  if nodeType == 'sign':
    return app.atfFromSign(n, flags=flags)
  elif nodeType == 'quad':
    return app.atfFromQuad(n, flags=flags, outer=True)
  else:
    return '«no outer quad»'


def atfFromCluster(app, n, seen=None):
  api = app.api
  F = api.F
  E = api.E
  ...
  for child in children:
    ...
    thisResult = (
        app.atfFromCluster(child, seen=seen)
        if childType == 'cluster' else app.atfFromQuad(child, flags=True) if childType == 'quad'
        else app.atfFromSign(child, flags=True) if childType == 'sign' else None
    )
    seen.add(child)
